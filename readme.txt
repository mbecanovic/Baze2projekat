U ovom projektu je radjena aplikacija koja simulira elektronske novine pomoću MERN tehnologije. Pomoću komitova mogu da se bide detalji koji su razradjeni prilikom build-ovanja pve aplikacije. 

Aplikacine je full stack, sto znaci da je uspešno omogućena komunikacija izmedju frontend-a i backend-a, kao i to da je backend uspešno povezan sa bazom. 
...
ps. ako korisite aplikaciju neophodno je u frontend folderu, kao i backend folderu pokrenuti npm install i time instalirati neophodne "pakete" za rad aplikacije kao sto su axios, react-router-dom, itd.
